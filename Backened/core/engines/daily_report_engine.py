...
class DailyReportEngine:
    ...
    def __init__(self) -> None:
        ...
        self.ingredients_path = base_dir / "data" / "shicai_enriched.csv"
        self.recipes_path = base_dir / "data" / "caipu_enriched.csv"
    ...
    def _load_data(self) -> None:
        with open(self.ingredients_path, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            for row in reader:
                # 处理可能包含 BOM 的表头
                row_id = row.get("id") or row.get("\ufeffid") or row.get("ï»¿id")
                row["id"] = int(row_id) if row_id else 0
        ...
        with open(self.recipes_path, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            for row in reader:
                row_id = row.get("id") or row.get("\ufeffid") or row.get("ï»¿id")
                row["id"] = int(row_id) if row_id else 0
