...
class RecommendEngine:
    ...
    async def _get_ai_reasons(self, constitution: str, solar_term: str, season: str, items: list, weather_data: Optional[dict] = None) -> dict:
        ...
        prompt = (
            f"体质: {constitution}, 节气: {solar_term}{weather_str}\n"
            f"项目: {items}\n"
            "为每个项目写50字专业中医理由，并检索或模拟一段相关的中医典籍原文(如《本草纲目》言...)。\n"
            "返回JSON格式: {\"项目名\": {\"reason\": \"理由\", \"quote\": \"典籍原文\"}}"
        )
        try:
            import time
            start = time.time()
            ...
            print(f"📖 [AI Debug] 成功获取RAG理由 (耗时 {time.time()-start:.1f}s)")
            return reasons
    ...
    def _get_fallback_reasons(self, constitution: str, solar_term: str, items: list) -> dict:
        ...
        for item in items:
            s_item = str(item)
            res_item = {"reason": "", "quote": ""}
            
            if s_item in self.tcm_knowledge_base:
                res_item["reason"] = self.tcm_knowledge_base[s_item]
                res_item["quote"] = f"《饮食须知》合『{solar_term}』时令推荐"
            else:
                effect_str = ""
                ancient_quote = ""
                # 首先在主食材库中查找
                for ing in ingredient_db.values():
                    if ing.get("name") == s_item:
                        ...
                        ancient_quote = ing.get("ancient_quote", "")
                        break
                # 如果没找到，在菜谱库中查找
                if not effect_str:
                    ...
                    for recipe in therapy_service.recipes:
                        if recipe.name == s_item:
                            ...
                            ancient_quote = getattr(recipe, 'ancient_quote', '')
                            break
                            
                res_item["reason"] = effect_str or f"对症调理{constitution}"
                res_item["quote"] = ancient_quote or f"依《本草纲目》合『{solar_term}』时令推荐"
            
            reasons[s_item] = res_item
        return reasons
    async def get_smart_recommendations(self, user_id: str, constitution: str, age: Optional[int] = None, gender: Optional[str] = None, weather_data: Optional[dict] = None):
        ...
        def get_fields(name):
            data = reasons_map.get(name, {})
            if isinstance(data, str):
                return data, ""
            return data.get("reason", ""), data.get("quote", "")
        ...
        return {
            ...
            "therapies": [
                RecommendationItem(
                    title=t, 
                    reason=get_fields(t)[0],
                    ancient_quote=get_fields(t)[1]
            ...
            "recipes": [
                RecommendationItem(
                    id=r.id, 
                    title=r.name, 
                    reason=get_fields(r.name)[0],
                    ancient_quote=get_fields(r.name)[1],
                    image=getattr(r, 'images', '')
            ...
            "ingredients": [
                RecommendationItem(
                    id=i.get('id', 0), 
                    title=i.get('name', ''), 
                    reason=get_fields(i.get('name', ''))[0],
                    ancient_quote=get_fields(i.get('name', ''))[1],
                    image=i.get('images', '')
