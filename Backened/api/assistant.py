...
async def chat_with_assistant(request: ChatRequest):
    ...
    matched_recipes = search_recipes_by_keyword(recipes, user_message)
    
    # 构建食谱数据上下文
    recipe_context = ""
    if matched_recipes:
        recipe_context = "根据我们的食疗数据库，为您推荐以下相关食谱：\n"
        for i, recipe in enumerate(matched_recipes[:3], 1):  # 最多推荐3个食谱
            recipe_context += f"{i}. {recipe.name}\n"
            recipe_context += f"   食材：{recipe.ingredients}\n"
            recipe_context += f"   功效：{recipe.effect}\n"
            recipe_context += f"   适用体质：{recipe.suitable}\n"
            recipe_context += f"   制作步骤：{recipe.steps}\n"
            recipe_context += f"   禁忌：{recipe.taboo}\n\n"
    else:
        recipe_context = "未找到相关食谱数据。\n"
