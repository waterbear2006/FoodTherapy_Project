...
- 从 shicai_enriched.csv 文件加载食材数据
"""
...
async def get_ingredient_by_name(name: str):
    """根据名称获取食材标准化详情"""
    if name not in ingredient_by_name:
        raise HTTPException(status_code=404, detail="该食材不存在")
    
    data = ingredient_by_name[name]
    return StandardResourceResponse(
        name=data["name"],
        summary=data.get("effect") or "暂无详细功效说明",
