...
function downloadReport() {
  ...
  const foodsHtml = (r.foods || []).map(f => `<span class="food-tag">${f}</span>`).join('')
  const therapiesHtml = (r.therapies || []).map(t => `<div class="therapy-item"><span class="therapy-icon">✓</span><span>${t}</span></div>`).join('')
...
.header { text-align: center; padding: 30px 20px; background: linear-gradient(135deg, rgba(144,180,148,0.85), rgba(118,156,122,0.85)); color: #fff; border-radius: 16px; margin-bottom: 20px; }
...
.card h2 { font-size: 18px; color: #5a7e5e; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 2px solid #e8f0e8; }
.constitution-type { font-size: 28px; font-weight: 700; color: #5a7e5e; text-align: center; margin: 10px 0; }
.constitution-desc { text-align: center; color: #666; font-size: 14px; margin-bottom: 16px; }
.feature { background: #f0f7f0; padding: 12px 16px; border-radius: 8px; border-left: 4px solid #5a7e5e; font-size: 14px; color: #444; }
.food-tags { display: flex; flex-wrap: wrap; gap: 8px; }
.food-tag { background: linear-gradient(135deg, rgba(144,180,148,0.85), rgba(118,156,122,0.85)); color: #fff; padding: 6px 14px; border-radius: 20px; font-size: 13px; }
.therapy-list { list-style: none; }
.therapy-item { display: flex; align-items: center; gap: 8px; padding: 8px 0; border-bottom: 1px solid #f0f0f0; }
.therapy-item:last-child { border-bottom: none; }
.therapy-icon { color: #5a7e5e; font-weight: 700; }
.user-info { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
.info-row { display: flex; padding: 8px 12px; background: #f8f8f8; border-radius: 8px; font-size: 13px; }
...
<div class="card">
<h2>🌿 体质类型</h2>
<div class="constitution-type">${r.type}${r.is_combination ? '（兼夹体质）' : ''}</div>
<p class="constitution-desc">${r.description || ''}</p>
</div>

<div class="card">
<h2>✨ 体质特征</h2>
<div class="feature">${r.feature}</div>
</div>

<div class="card">
<h2>🍲 饮食调理建议</h2>
<p style="font-size:14px;color:#444;">${r.diet}</p>
</div>

<div class="card">
<h2>🥬 推荐食材</h2>
<div class="food-tags">${foodsHtml}</div>
</div>

<div class="card">
<h2>👨‍ 推荐食疗方案</h2>
<div class="therapy-list">${therapiesHtml}</div>
</div>

<div class="card">
<h2>👤 基本信息</h2>
<div class="user-info">
<div class="info-row"><span class="info-label">性别：</span><span class="info-value">${userInfo.value.gender || '-'}</span></div>
<div class="info-row"><span class="info-label">年龄：</span><span class="info-value">${userInfo.value.age || '-'}岁</span></div>
<div class="info-row"><span class="info-label">身高：</span><span class="info-value">${userInfo.value.height || '-'}cm</span></div>
<div class="info-row"><span class="info-label">体重：</span><span class="info-value">${userInfo.value.weight || '-'}kg</span></div>
...
<p>仅供参考，具体调理方案请咨询专业中医师</p>
  ...
  a.href = url
  a.download = `体质测试报告_${r.type}_${today.replace(/\//g, '-')}.html`
...
.btn-primary {
  ...
  font-size: 16px;
  font-weight: 600;
  box-shadow: 0 4px 12px rgba(118, 156, 122, 0.3);
  transition: all 0.2s ease;
}
...
.btn-submit {
  ...
  box-shadow: 0 6px 16px rgba(118, 156, 122, 0.3);
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 8px;
...
.btn-submit:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 8px 24px rgba(118, 156, 122, 0.4);
}
